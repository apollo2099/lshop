			...
			if(submitDivLoginValidate()){
				...
				$.post("/web/userCenterManage!login.action",$("#divlogin").serialize(),function(str){
					...
					if(f==1){
						...
						$("#msg").html("Email or nickname does not exist");
					}else if(f==-2){
...
var codeStatu = 0;
	...
	function validateEmail(){
		...
		if(email==dvalue){
		 ...
		 if(email==''){
			 ...
			 $("#email_tip2").text("Please enter your email");
	...
	function validateLogAccount(){
		...
		var name = $("#name").val();
		name = $.trim(name);
		if(name=='your email or nickname'){
			name='';
		}
		if(name==''){
			$("#name_tip2").text("Please enter your email or nickname");
			$("#name_tip2").css("display","block");
			$("#name").val('your email or nickname');
			return false;
		}
		return true;
	}
	...
	function submitDivLoginValidate(){
		...
		pwd = $.trim(pwd);
		if(uname == ""){
			$("#msg").text("Please enter your email or nickname");
			$("#erro_msg").css("display","block");
			return false;
		}
		if(pwd == ""){
	...
	function submitFindPwdValidate(){
		...
		code = $.trim(code);
		if(email == ""){
			$("#pwd_tip").text("Please enter your email or nickname");
			return false;
		}
		if(checkEmail(email)==false){
