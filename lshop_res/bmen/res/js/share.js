(function() {
	var conf = {
		weibo: {
			...
			icon: 'http://res.bananatv.com/bmen/res/pan/images/wb.gif',
			text: 'Weibo'
		},
		weixin: {
			icon: 'http://res.bananatv.com/bmen/res/pan/images/wx.gif',
			text: 'WeChat'
		}
	}
	...
	var activityShare = {
		...
		shareWeixin: function(actid, link){
			...
			$('#c_wx_publish').click(function(){
				...
				html += '<h3 style="line-height: 30px;margin: 0;font-weight: normal;">Share on WeChat</h3></div>';
				...
				html += '"/></div><div style="text-align: left;margin: 0 15px;padding: 0;font-size: 12px;">Go to "WeChat-Discover-Scan QR Code" and share the webpage to your WeChat Contacts or on WeChat Moments.</div>';
				html += '</div></div>';
				$('body').append(html);
			});
		}
