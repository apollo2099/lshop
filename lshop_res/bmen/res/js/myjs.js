 ...
 * js.defer(e, msel, fun)	将代码延迟执行，e为线程对象（一般为this），msel为暂停的时间(以毫秒为单位), fun为延迟执行的函数。
 * 
 * 
 * 
...
var defer = window["js"]["defer"] = function(e, msel, fun) {
	e.next = fun;
	window.currentThread = e;
	setTimeout("deferAfter()", msel);
}
function deferAfter() {
	var e = window.currentThread;
	if (e.next != undefined) {
		e.next();
	}
}
