...
window.onload=function(){
	...
	;(function(){
		objRotateImg.addEventListener("touchstart",function(e){
			e.stopPropagation();
			e.preventDefault();
			if(e.target==objRotateImg){
			deg+=90;
		    //objImg.style.transform="rotate("+deg+"deg)";
			//objImg.style.cssText+=("-webkit-transform:rotate("+deg+"deg);");
			var i=0;
			function rotate(){
				objRotateImg.style.webkitTransform="rotate("+i+"deg)";
				i++;
			}
			var objInterval=setInterval(rotate,10);
			function clearRotate(){
				clearInterval(objInterval);
				objRotateImg.style.webkitTransform="rotate(-"+i+"deg)";
			}
			setTimeout(clearRotate,500);
			objImg.style.webkitTransform="rotate("+deg+"deg)";
			item=((deg%360)/90)%2;
	...
	;(function(){
		objIcon.addEventListener("touchend",function(e){
			...
			e.preventDefault();
			drag=0;
			var oL=objImg.offsetLeft,oT=objImg.offsetTop;
			if(item==0){
			    objHiddenP.value=oL+","+oT;
			}
			else if(item==1&&$$("img")){
				var obj=$$("img");
				var w=parseInt(obj.offsetWidth/2),h=parseInt(obj.offsetHeight/2);
				objHiddenP.value=(oL+w-h)+","+(oT+h-w);
			}
		},false);
	}());
...
//	
	...
	var objProgress=$$("progress"),objPercent=$$("percent");
	objSub.onclick=function(){
		if(recipientPerson.innerHTML=="添加收件人"||sentimentTxt.value==""||senderPerson.innerHTML=="添加发件人"){
			...
			var H=document.documentElement.offsetHeight,T=document.body.scrollTop;
			objProgress.style.height=H+"px";
			objPercent.style.marginTop=parseInt((H-T)/2)+T+"px";
			uploadImag();
		}
	}
...
function setDesc(){
	...
	if(sentimentTxt1.value!=""){
        ...
        var textnum=parseInt(ulw/fontw)+1;
