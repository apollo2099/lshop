...
<jsp:param value="lvOrderAction!list.action?json.navTabId=${json.navTabId}" name="pagerFormAction"/>
</jsp:include>
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="lvOrderAction!list.action?json.navTabId=${json.navTabId}" method="post">
	<div class="searchBar">
	        ...
	        <input type="hidden" name="lvOrder.isSyncSas" value="${lvOrder.isSyncSas }">
			...
			<table class="searchContent" >
				<tr>
						...
						<td><label>第三方订单号</label><input type="text" name="lvOrder.thirdOrderNum"  value="${lvOrder.thirdOrderNum }"/></td>
				</tr><tr>
				</tr><tr>
					  <td><label>订单类型</label>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			...
			<li><a class="add" href="lvOrderAction!befOrderImport.action?json.navTabId=${json.navTabId}" target="dialog"  width="850" height="500" title="第三方订单批量导入" mask="true"><span>批量导入</span></a></li>
			<li class="line">line</li>
			</c:if>
	...
	<table class="table" width="100%" layoutH="235">
		<thead>
			<tr>
				...
				<th width="3%">支付状态</th>
				<th width="3%">订单状态</th>
				<th width="7%" orderField="createTime" class="${orderDirection}">下单时间</th>
				<th width="7%" orderField="overtime" class="${orderDirection}">支付时间</th>
				<th width="25%">操作</th>
				<th width="5%">发货备注</th>
				<th width="5%">退货备注</th>
			</tr>
     ...
     <s:iterator value="page.list" status="stat" id="item">
			<tr>
				...
				<td>
				<s:date name="overtime" format="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
				<sec:authorize url="viewOrderDetails">
				    <a class="btnView" href="lvOrderAction!view.action?lvOrder.id=${item.id}&lvOrder.oid=${item.oid}&json.navTabId=${json.navTabId }" target="dialog" navTabId="lvAccount" rel="lvAccount" title="查看"  width="900" height="750" mask="true">查看</a>
                </sec:authorize>
				...
				<sec:authorize url="deleteOrderInfo">
				    ...
				    <c:if test="${item.status==0 and item.payStatus==1 and item.isServiceAudit==1 and item.isFinanceAudit==1 and item.isdelete!=-1 and item.isSyncSas==1} ">
				...
				<sec:authorize url="orderBreakRemark">
				    <c:if test="${item.status==3 and item.isdelete!=-1}">
				       <a  href="lvOrderAction!befBreakRemark.action?lvOrder.id=${item.id}&json.navTabId=${json.navTabId }"  title="修改退货备注" target="dialog"  navTabId="lvProduct" rel="lvProduct" width="420" height="300" mask="true">退货备注</a>
				    </c:if>
				</sec:authorize>
				...
				<sec:authorize url="orderSyncSas">
				<c:if test="${item.isSyncSas==0 and item.isdelete!=-1 and lvOrder.isFinanceAudit==1 and lvOrder.isServiceAudit==1}">
				  <a href="lvOrderAction!synSendSasOrder.action?lvOrder.oid=${item.oid}&json.navTabId=${json.navTabId }"  title="同步启创" target="ajaxTodo" mask="true">同步启创</a>
				</c:if>
				</sec:authorize>
