...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="lvProductAction!list.action" method="post">
		<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		...
		<sec:authorize url="userEdit">
		    <li><a class="add" href="lvProductAction!befAdd.action?json.navTabId=${json.navTabId }" target="navTab" navTabId="lvProduct" rel="lvProduct" mask="true"  title="发布产品"><span>发布产品</span></a></li>
		...
		<s:iterator value="page.list"  id="product">
			<tr>
				...
				<sec:authorize url="userEdit">
					...
					<a href="lvProductAction!getProductLadder.action?lvProduct.code=${product.code}&lvProduct.storeId=${product.storeId }&json.navTabId=lvShopSubject_2"  rel="lvShopSubject_2" target="navTab">阶梯价</a>
					<c:if test="${product.isSetMeal!=1}">
					   <a href="lvProductAction!commendList.action?lvProduct.code=${product.code}&lvProduct.storeId=${product.storeId }&json.navTabId=commendList_1"  rel="commendList_1" target="navTab">推荐组合</a>
					</c:if>
					<c:if test="${product.isSupport!=0}">
					   <a href="lvProductAction!updateSupport.action?lvProduct.code=${product.code}&lvProduct.isSupport=0&json.navTabId=${json.navTabId }"  title="确实将改产品下架吗?该操作请谨慎!" target="ajaxTodo">下架</a>
					</c:if>
					
					<a href="lvProductAction!updateCache.action?lvProduct.code=${product.code}&json.navTabId=${json.navTabId }"  title="确定更新缓存？" target="ajaxTodo" style="color:red">更新缓存</a>
