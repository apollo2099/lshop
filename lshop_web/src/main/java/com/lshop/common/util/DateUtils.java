...
public abstract class DateUtils extends org.apache.commons.lang.time.DateUtils {
	 ...
	 * @Description:大于等于
	 * @author CYJ
	 * @date 2014-7-1 下午5:05:36
	 * @param src
	 *            源日期
	 * @param target
	 *            目标日期
	 * @return 源大于等于目标返回true
	 */
	public static boolean ge(Date src, Date target) {
		boolean result = false;
		int i = src.compareTo(target);
		if (i == 0 || i == 1) {
			result = true;
		}
		return result;
	 ...
	 * @param target
	 *            目标日期
	 * @return 源小于等于目标返回true
	 */
	public static boolean le(Date src, Date target) {
		boolean result = false;
		int i = src.compareTo(target);
		if (i == 0 || i == -1) {
			result = true;
