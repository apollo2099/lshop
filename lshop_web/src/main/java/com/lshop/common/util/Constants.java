...
public class Constants {
	...
	//域名信息(店铺标志对应域名)
	public static Map<String,String> DOMAIN_STORE = new HashMap<String,String>();
	//店铺标志对于店铺id信息
	...
	//店铺标志对于币种关系
	public static Map<String,String> STORE_TO_CURRENCY = new HashMap<String,String>();
	//店铺标志对于店铺名称
	public static Map<String,String> STORE_TO_NAME = new HashMap<String,String>();
	
	//使用中的店铺标识
	public static List<String> STORE_IN_USE = new ArrayList<String>();
	...
	public static short PAY_METHOD_WORLDPAY=18;//wroldpay支付
	
	public static short PAY_METHOD_ALIPAY_MOBILE = 18;	//手机支付宝
	public static short PAY_METHOD_ALIPAY_INITIAL = 19;	//支付宝国际卡银行支付
	public static short PAY_METHOD_QH = 20;	//钱海支付
	...
	static{
		...
		PAY_METHODS.put(PAY_METHOD_ALIPAY_MOBILE, "手机支付宝");
		PAY_METHODS.put(PAY_METHOD_ALIPAY_INITIAL, "支付宝国际银行卡");
		PAY_METHODS.put(PAY_METHOD_QH, "钱海支付");
	}
	...
	public static float rateNumCNY=0.68f;//人民币对美元的汇率
	public static float rateVbNum=0.1f;//虚拟货币V币与状元的汇率
	public static float rateVbNumCny=0.63f;//虚拟货币V币与人民币的汇率
	
	public final static int PAYTYPE_LINEPAY = 0;	// 支付类型：在线支付
	public final static int PAYTYPE_LINEPAY_VB = 1;	// 支付类型：在线充值
	
	public static String baseDomain = (String)PropertiesHelper.getConfiguration().getProperty("domain.base.name");
	public static Map<String,String> sysConfigParam = new HashMap<String, String>();
	...
	/**加载配置项*/
	private static void initSysConfigParam(){
		HibernateBaseDAO lvlogicReadDao = (HibernateBaseDAO)ApplicationContextHolder.getBean("lvlogicReadDao");
		List<LvSysConfig> list = lvlogicReadDao.find("from LvSysConfig where status<>-1", null);
		for(LvSysConfig c : list){
			/**配置类型 0公共配置 1为tvpad项目 2为banana项目*/
			String name = null;
			if(baseDomain.equals(StoreMsgUtil.TVPAD_BASEDOMAIN)){
            	if(c.getType() == 2){continue;}else
            	if(c.getType() == 1){name = StoreMsgUtil.TVPAD_PRE + c.getName();}else
            	{name = c.getName();}
            	sysConfigParam.put(name, c.getVal());
            }else if(baseDomain.equals(StoreMsgUtil.BANANATV_BASEDOMAIN)){
            	if(c.getType() == 1){continue;}else
            	if(c.getType() == 2){name = StoreMsgUtil.BANANATV_PRE + c.getName();}else
            	{name = c.getName();}
            	sysConfigParam.put(name, c.getVal());
            }
		}
	...
	public static class ConfigParam{
		/**根据key获取公共配置项*/
		public static String getCommonConfigParam(String key){
			return sysConfigParam.get(key);
		}
		/**根据key获取商城配置项*/
		public static String getMallConfigParam(String key){
			if(baseDomain.equals(StoreMsgUtil.TVPAD_BASEDOMAIN)) return sysConfigParam.get(StoreMsgUtil.TVPAD_PRE+key);
			if(baseDomain.equals(StoreMsgUtil.BANANATV_BASEDOMAIN)) return sysConfigParam.get(StoreMsgUtil.BANANATV_PRE+key);
			return "";
		}
	}
}
