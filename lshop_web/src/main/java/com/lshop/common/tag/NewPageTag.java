...
public class NewPageTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspException {
		...
		if(null == href || href.equals("")){
			this.href = (String)request.getAttribute("pageurl");
		}
		...
		if (pagination != null) {
			...
			if(language.equals("en")){
				html.append("<p class='jumpto'>Jump to</p><p><input id='pageValue' name='pageValue' type='text' class='pro_bd' maxlength='3'/></p>");
			}else{
				html.append("<p class='jumpto'>跳转到</p><p><input id='pageValue' name='pageValue' type='text' maxlength='3' class='pro_bd'/></p>");
			}
			html.append("<p><input name='' type='button'  class='button03' onclick='toPage();' value='GO' style='cursor:pointer;'/></p>");
			html.append("</div>");
