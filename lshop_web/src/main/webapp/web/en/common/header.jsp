    		...
    		<div id="help">
          		...
          		<%--
		            <li class="yh" id="loginId"><a href="javascript:onshow('tx_b','loginDiv');">Login</a></li>
		             --%>
		            <li class="yh" id="loginId"><a href="javascript:toLogin('${storeDomain }');">Login</a></li>
   			...
   			<div class="search">
				<img id="c_search_coll"
   				...
   				<c:if test="${not empty searchContent}">
					...
					src="${resDomain}/en/res/images/src_bt.png">
				<div id="c_inner_search" class="inner_srarch" <c:if test="${not empty searchContent}">style="display:block;"</c:if> >
				<form action="${storeDomain}/web/store!searchProducts.action" method="post" id="searchForm">
			  		<input name="str" type="text" maxlength="50" class="sc_wb" id="searchContent" <c:if test="${not empty searchContent}">value="${searchContent}"</c:if><c:if test="${empty searchContent}">value="TVpad4"</c:if>  />
