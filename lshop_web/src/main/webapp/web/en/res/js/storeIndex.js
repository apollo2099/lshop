/*
			...
			function getShopProducts(){
		   		if(shopData[0].length>=1){
		  			...
		  			$.each(shopData[0],function(n,obj) {
						...
						if(obj[1].length>4){
							str += "<span class='more'><a href='/web/store!showMoreProducts.action?subjectType="+obj[0].code+"&exhibitType="+obj[0].exhibitType+"'>More>></a></span>"
						}
						str += "</h2>";
						if(obj[0].exhibitType==1){
							...
							$.each(obj[1],function(m,ob){
								if(m<4){
									...
									str += "<p><span class='mingcheng'>"+ob[0].productName+"</span><span class='price'>Price：<span class='price1'>USD "+ob[0].productPrice+"</span></span><span class='country'>"+ob[1].country+"</span><span class='city'> - "+ob[1].city+"</span></p>";
									str += "</a>";
									str += "</li>";
								}
							});
			...
			function getStores(){
				if(shopData[1].length>=1){
					var str = "";
					str += "<h2 class='bt2'><p class='cx'>Recommended Store<span class='yw'></span></p></h2>";
					...
					$.each(shopData[1],function(n,store) {
						if(n<8){
							...
							str += "<p><span class='name'><a href="+tmp+">"+store.name+"</a></span><span class='price'><a href="+tmp1+">"+store.country+"</a> - <a href="+tmp2+">"+store.city+"</a></span></p>";
							str += "</li>";
						}
					});
					str += "<div class='cb'></div>";
