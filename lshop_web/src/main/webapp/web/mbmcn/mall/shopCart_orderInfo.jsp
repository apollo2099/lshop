...
<title>banana商城_填写订单</title>
<meta charset="utf-8">
<meta content="width=device-width,user-scalable=no" name="viewport">
<link href="${resDomain}/mbmcn/res/css/style.css" rel="stylesheet" type="text/css">
<script type="text/javascript" src="${resDomain}/mbmcn/res/js/jquery-1.10.2.min.js"></script>
...
<script type="text/javascript" src="${resDomain}/mbmcn/res/js/dev.js"></script>
	...
	<c:if test="${not empty addressList}">
		<c:foreach items="${addressList}" var="address" varStatus="status">
			...
			<div class="massinfomaw">
				<div class="conmaseg" addresscode="${address.code}" deliveryid="${deliverId}" storeflag="${lvStore.storeFlag}" allamount="${allAmount }">
...
<article class="add_addr" style="display:none;">
	<form action="/web/shopCart!addAddress.action" method="post" id="addressForm">
	<input type="hidden" name="shopFlag" value="${lvStore.storeFlag}"/>
							...
							<div>
								<table width="100%" border="0">
									<tr>
										...
										<td valign="middle" class="imgjiag">${lvStore.currency} ${obj[0].shopPrice }</td>
									</tr>
								</table>
							</div>
						</div>
...
$('#addr_ls_area').on('click', '.conmaseg', function(e){
	...
	$.ajax({
	   ...
	   data: "deliveryId="+deliveryid+"&shopFlag="+shopFlag,
...
$('#addr_ls_area').on('click', '.addr_edit_btn', function(e){
	...
	$.post('/web/shopCart!getAddressJson.action', {myAddress: eaddr}, function(data){
		...
		$('#c_contry').val(data.contryId).trigger('change');
		$('#contrynameId').val(data.contryName);
		//省
		var $pv = $('#c_province');
		if($pv.is(':visible')){
			$pv.val(data.provinceId).trigger('change');
		...
		var $ct = $('#c_city');
		if($ct.is(':visible')){
			$ct.val(data.cityId).trigger('change');
...
function beforeFormSubmit(){
	$('#c_contry:visible').trigger('valiField');
