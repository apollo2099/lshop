...
/**	 $.ajax({
	   type: "POST",
	   url: "/web/store!saveShopCookie.action",
	   async:false,
	   success: function(){
	   		//从cookie中获取已选择的店铺信息
			 var store = lshop.getCookieToJSON('store');
			 if(store.name!=null){
			 	$("#selectStore").html("前往&nbsp;<a href='http://"+store.domainName+"/index.html' target='_blank'><span class='blue'>"+store.name+"</span></a>");
			 }else{
...
$(function(){
	$.ajax({
	   type: "get",
	   url: "${resDomain}/tvpadcn/res/js/storeArea.js",
	   dataType:"JSON",
	   async:false,
...
function toCar(storeDomain){
		window.location.href=storeDomain+"/web/mall!getShopCartList.action";
}
...
function checkHeaderEmail() {
	...
	if (js.assert.isIllegalLength("h_uname", "<2||>32", '賬號只能是2至32位字符！', "h_emsg")) {return false;}
	return true;
}
