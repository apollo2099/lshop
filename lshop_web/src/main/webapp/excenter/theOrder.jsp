...
<div class="main_conten3">
	<!--我的推广码-->
	<div class="theorder">
		<ol class="f_B">推广码<span class="f_orange"> ${param.couponCode }</span> 的推广订单详细情况</ol>
		<ul>
			<li class="my_od">
				<p class="od01">买家姓名</p>
				<p class="od02">推广码</p>
		    ...
		    <c:foreach items="${pagination.list}" var="obj">
			<li>
				...
				<p class="od06">
				     <c:if test="${obj[5]=='2'}">已结算</c:if>
				     <c:if test="${obj[5]=='1'}">待结算</c:if>
				     <c:if test="${obj[5]=='0'}">未结算</c:if>
				</p>
			</li>
			</c:foreach>
		</ul>
		<ul class="page" style="float:right;">
				  <u:page href="/excenter/DiffusionCode/promtManager!getSettledList.action?page.pageNum=@"></u:page>
				   <script type="text/javascript">
					<!--
					function gotoPage(num){
					   window.location.href="/excenter/DiffusionCode/promtManager!getSettledList.action?page.pageNum="+num;
